...
class fm_admin_settings {
	...
	public function css_button() {
		...
		<section class="css-button-radius">
			...
			<p style="vertical-align: middle; display:table-row;"><label for="fmcta-css-button-radius-top-left"
			                                                             style="display:table-cell; width: 100px;">Top
					Left</label>
				<input type="text" id="fmcta-css-button-radius-top-left" style="display:table-cell;"
				       name="fmcta_settings[css_button_radius-top-left]"/></p>

			<p>
				<label for="fmcta-css-button-radius-top-right">Top Right</label>
				<input type="text" id="fmcta-css-button-radius-top-right"
				       name="fmcta_settings[css_button_radius-top-right]"/>
			</p>
		</section>
		<?php
		//);
	...
	public function fmcta_settings_post_types() { ?>
		<section class="post-types">
			...
			<p class="description">By default all post types are included. If you don't want a post type to display in
				the feature me widget, select it here:</p>
			<?php
			$post_types = get_post_types( array(
					'public' => true, //only show public post types
				)
			);
			unset( $post_types['attachment'] ); // Remove Attachments as an option.
